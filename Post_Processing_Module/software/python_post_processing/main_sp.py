...
def sub_mistake(word, char_err, char_sub):
    ...
    return word
# w = "l'humanite'"  # melme feul perlbnne confiderables reniarquable basseffe naiffance,
...
def mergeTwoWord(nextLine, word, firstWordNextLine):
    ...
    print("about to delete the next line", nextLine)
    if nextLine:
        del nextLine[0]
...
def corrErrOneW(word):
    ...
    for seq in wrong_e:
        if seq in word:
            # word = word.replace(seq, "é")
            return word.replace(seq, "é")
    wrong_apostrophe = [".", "s", "a"]
    for error in wrong_apostrophe:
        for char in vowels:
            if (error + char) in word: #and word[2].find(vowels): #a vowel follows this sequence
                charList = list(word)
                charList[1] = "'"
                word = "".join(charList)
                return word
...
def inDictError(word, next_line=None, first_word_next_line=None):
    ...
    if not next_line and not first_word_next_line: # made a change here** potential error
        ...
        if atEndofLine(word, currLine):
            if hasEndPunct(word):
                if isMergeable(word, firstWordNextLine):
                    ...
                    # print("THIS IS THE WORD BEFORE ERR_EOL", word)
                    temp = inDictErrEOL(nextLine, word, firstWordNextLine)
                    if temp:
                        # print("THIS IS THE WORD GENERATED FROM ERR_EOL", temp)
                        correctLine = correctLine + temp + " "
...
def prepPage(pageAsList):
    ...
    # and any lines which exceed the line limit
    ...
    if nextPage:
        for lineNo in range(len(nextPage) - 1, -1, -1):
            ...
            if not nextPage[lineNo]:
                del nextPage[lineNo]
        print("if this is the current page length", len(currPage))
        ...
        print("else this is the current page length", len(currPage))
        for lineNo in range(0, len(currPage)):
            if not currPage[lineNo]:
                del currPage[lineNo]
        for lineNo in range(0, len(currPage)):
            ...
            if lineNo < len(currPage)-1:
                nextLine = currPage[lineNo+1]
                print("this is the next line (line 360 in code", nextLine)
                firstWordNextLine = nextLine[0]
            pageTxt = pageTxt + flowchart(currLine, nextLine, firstWordNextLine) + "\n"
    return pageTxt
...
def main():
    ...
    output = "../../4_Output_Final_Text/" + chosen_folder
    dirs = os.listdir(directory)
    for i, file in enumerate(dirs):
        print("this is the current page", dirs[i])
        if tempPage:
            currPage = tempPage
            for line in currPage:
                    print(line)
        else:
            path = directory + "/" + dirs[i]
            with open(path, 'r', encoding="utf8") as fOpen:
                currPage = fOpen.readlines()
                print("about the print stuff")
                for line in currPage:
                    print(line)
